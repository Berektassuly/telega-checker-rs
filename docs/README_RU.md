...
Высокопроизводительный Telegram-бот для обнаружения аккаунтов, скомпрометированных MITM-форком **Telega**. Написан на Rust. Принимает Telegram user ID через личное сообщение или inline-запрос и возвращает бинарный результат: присутствует или отсутствует в VoIP-инфраструктуре Telega.
...
Бот реализует трёхуровневый поиск с различными характеристиками задержки и персистентности на каждом уровне.
...
└── src/
    ├── main.rs             # Точка входа: конфиг, пул, кеш, настройка диспетчера
    ...
    ├── bot_handler.rs      # Telegram-обработчики: /start, сообщения, inline-запросы; трёхуровневый поиск
    └── db.rs               # Инициализация схемы SQLite, CRUD known_users, аналитическое логирование
```
